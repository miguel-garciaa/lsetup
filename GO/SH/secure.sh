...
if [ ! -f "$USER_HOME/.ssh/authorized_keys" ] || [ ! -s "$USER_HOME/.ssh/authorized_keys" ]; then
  echo "[WARN] ALERTA CRÍTICA: El usuario '$SSH_USER' no tiene claves en '$USER_HOME/.ssh/authorized_keys'."
  exit 1
fi
...
if [[ -n "$LARAVEL_DIR" ]]; then
  echo "   [OK] Proyecto Laravel detectado: $LARAVEL_DIR (se parcheará .env con REDIS_PASSWORD)"
else
...
#!/bin/bash
echo -e "\n======================================================="
echo -e "  PANEL DE AUDITORÍA Y SEGURIDAD (SECOPS)"
echo -e "=======================================================\n"
...
if [ -f /var/lib/.system-state/logs/state.txt ]; then
  ...
  while IFS='=' read -r k v; do
      case "$k" in
          ...
          last_run_ok)     [ "$v" = "1" ] && echo "    Resultado:  [OK] OK" || echo "    Resultado:  [ERROR] FALLÓ";;
  ...
  echo "  [WARN]  No hay runs registrados. ¿Está instalado?"
...
if [ -f /var/lib/.system-state/logs/verify.state ]; then
  ...
  awk -F= '/last_verify_tag/{print "    TAG: "$2}
           /last_verify_ok/{if($2==1)print "    VERIFY:    OK";else print "    VERIFY:    [ERROR] FAIL"}
...
if [ ! -f "$REDIS_CONF" ]; then
  echo "   [WARN]  $REDIS_CONF no existe. ¿Instalado Redis? Skip."
else
...
if mount | grep -qE 'on /tmp type tmpfs'; then
  ...
  if ! grep -qE '^[^#]*[[:space:]]/tmp[[:space:]]' /etc/fstab; then
      ...
      echo "   [WARN]  Antes de reboot: prueba 'sudo dnf update' (algunos paquetes usan /tmp)."
  fi
fi
...
if [ -f /etc/dnf/automatic.conf ]; then
  ...
  echo "   [WARN]  /etc/dnf/automatic.conf no existe; paquete dnf-automatic no instalado. Skip."
fi
...
if systemctl is-active --quiet postgresql-18 2>/dev/null; then
  ...
  echo "   [WARN]  postgresql-18 no activo. Skip hardening PG (ejecuta secure.sh tras setup.sh)."
fi
...
if ! visudo -cf "$SUDO_TMP" >/dev/null 2>&1; then
  echo "   [WARN]  Sintaxis sudoers inválida para '$SSH_USER'. Eliminando temporal. NO se aplica."
  rm -f "$SUDO_TMP"
else
  install -m 440 -o root -g root "$SUDO_TMP" "$SUDO_FILE"
  rm -f "$SUDO_TMP"
  # Validación final global (las otras reglas sec-logs).
  visudo -c &>/dev/null || { echo "   [WARN]  visudo -c global falla tras instalar $SUDO_FILE. Revísalo."; }
fi
...
# El binario estático FrankenPHP embebe su propio PHP + exts: el WORKER
...
; FrankenPHP worker NO se ve afectado (embeds propio PHP).
...
EOF
  ...
  echo "   [WARN]  /etc/php.d no existe. PHP CLI no instalado. Skip."
fi
...
EOF
      ...
      if ! nginx -t 2>/tmp/nginx_err4; then
          echo "   [WARN]  AVISO: nginx -t falló (sección 28 COEP). Detalle:"
          cat /tmp/nginx_err4
      else
...
EOF
  ...
  if [ "$REDIS_UP" != "yes" ]; then
      echo "   [WARN]  Redis no levantó tras ACL. Revisa config:"
...
if systemctl is-active --quiet postgresql-18 2>/dev/null; then
  ...
  if [[ -n "$LARAVEL_DIR" && -f "$LARAVEL_DIR/.env" ]]; then
      ...
      if ! [[ "$PG_APP_DB" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then
          echo "   [WARN]  DB_DATABASE '$PG_APP_DB' inválido. Uso laravel1."
          PG_APP_DB="laravel1"
      fi
  fi
  ...
  if [ ! -f "$PG_STAT_SO" ]; then
      echo "   [WARN]  contrib no instalable. Skip shared_preload_libraries (pg_stat_statements NO activo)."
  else
      ...
      if ! echo "$CURRENT_PRELOAD" | grep -q 'pg_stat_statements'; then
          ...
          if [ "$PG_UP" != "yes" ]; then
              echo "   [WARN]  PG no levantó tras preload. Rollback de ALTER SYSTEM..."
              ...
              systemctl is-active --quiet postgresql-18 \
                  || echo "   [WARN]  PG sigue caído. Revisa: journalctl -u postgresql-18 --no-pager | tail -40"
          fi
      fi
  fi
...
echo "  - FrankenPHP:   NO tocado (embeds propio PHP)"
...
echo " [WARN]  TMPFS:        si /tmp estaba en disco, reboot para aplicar noexec/nosuid/nodev"
echo "=========================================================================="
...
maybe_reboot() {
  ...
  if [ "$WHEN" = "now" ]; then
      ...
      echo "   [WARN]  Formato no reconocido. No programado. Reboot manual."
  fi
}
...
if [ -n "$REBOOT_RAZONES" ]; then
  ...
  echo " >> No hay cambios pendientes de reboot. Todo aplicado en caliente. [OK]"
fi
