...
func printHelp() {
	...
	fmt.Println("  [harden]        Opcional. enc_confirm (s=aplicar SESSION_ENCRYPT default, n=posponer).")
	fmt.Println("                  Vacía → up aplica default s silencioso (sin prompt).")
	fmt.Println("  [secure]        Opcional. allowed_ip, ssh_port, ssh_user, redis_pass (REQUERIDOS),")
	fmt.Println("                  report_email, reboot_schedule. Vacía → up skip hardening.")
