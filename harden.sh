...
if [ -z "$LARAVEL_DIR" ]; then
    echo "❌ No se detectó proyecto Laravel bajo /var/www. Aborto (este script es app-layer)."
    exit 1
fi
...
if [ -d "$MODELS_DIR" ]; then
    ...
    while IFS= read -r model; do
        ...
        if grep -qE '\$guarded[[:space:]]*=[[:space:]]*\[\][[:space:]]*;' "$model" 2>/dev/null; then
            echo "   🚨 $fname: \$guarded = [] (mass-assignment abierto)"
            GUARDED_ISSUES=$((GUARDED_ISSUES + 1))
        elif ! grep -qE '\$fillable[[:space:]]*=' "$model" 2>/dev/null; then
            echo "   ⚠️  $fname: sin \$fillable definido (revisar asignación manual)"
...
if [ "$CURRENT_DEBUG" != "false" ]; then
    echo "   ⚠️  APP_DEBUG=$CURRENT_DEBUG → forzando false (prod OBLIGATORIO)."
...
if [ -f "$ROUTES_WEB" ]; then
    grep -qE 'throttle:5,1' "$ROUTES_WEB" 2>/dev/null \
        && echo "     ✅ routes/web.php tiene throttle:5,1" \
        || echo "     ⚠️  routes/web.php SIN throttle:5,1 (añade ->middleware('throttle:5,1') a /login y /password/*)"
else
    echo "     - routes/web.php no encontrado."
fi
if [ -f "$ROUTES_API" ]; then
    grep -qE 'throttle:60,1|throttle:[0-9]+,1' "$ROUTES_API" 2>/dev/null \
        && echo "     ✅ routes/api.php tiene throttle" \
        || echo "     ⚠️  routes/api.php SIN throttle (añade ->middleware('throttle:60,1') al grupo api)"
else
...
if grep -q 'google' "$ROUTES_WEB" 2>/dev/null; then
    grep -qE 'auth/google.*throttle|password/reset.*throttle' "$ROUTES_WEB" 2>/dev/null \
        && echo "     ✅ rutas google/reset con throttle" \
        || echo "     ⚠️  auth/google/callback o password/reset SIN throttle (recomendado throttle:5,1)"
fi
...
if [ "$SKIP_OCTANE" -eq 1 ]; then
    ...
    systemctl restart octane 2>/dev/null && echo "   >> octane reiniciado (secrets via EnvironmentFile)." \
        || echo "   ⚠️  octane NO reinició. Revisa: systemctl status octane. Rollback: rm drop-in + daemon-reload."
else
...
echo "=========================================================================="
echo " ⚠️  PANIC GUARD: NO se ha tocado APP_KEY (rotate invalida TODO)."
echo " ⚠️  EnvironmentFile es MIRROR: .env conserva secretos (artisan CLI)."
