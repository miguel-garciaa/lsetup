...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  logecho "[STOP] Otro backup.sh corriendo (flock denial). Aborto."
  exit 1
fi
...
mover_pending() {
  ...
  if [ ! -s "$archivo" ]; then
      logecho "[ERROR] [$tipo] archivo vacío. NO se mueve."
  ...
  logecho "[OK] [$tipo] $(basename "$archivo") ($(numfmt --to=iec "${SIZES[$tipo]}" 2>/dev/null || echo "${SIZES[$tipo]}b"))"
}
...
if [ -n "${LARAVEL_DIR:-}" ] && [ -f "$LARAVEL_DIR/.env" ]; then
  ...
  [ -z "$DB_NAME" ] && { logecho "[WARN]  DB_DATABASE vacío en .env. Skip backup DB."; }
  [ -z "$DB_USER" ] && { logecho "[WARN]  DB_USERNAME vacío en .env. Skip backup DB."; }
else
  logecho "   [WARN]  No se detectó Laravel. Solo se hará snapshot de configs (.keyring)."
  LARAVEL_DIR=""
fi
...
if [ -n "${DB_NAME:-}" ] && [ -n "${DB_USER:-}" ]; then
  ...
  if systemctl is-active --quiet postgresql-18 2>/dev/null; then
      ...
      then
          ...
          logecho "[ERROR] [db] pg_dump falló. Detalle: $(cat "$TMP_DIR/pd.err" 2>/dev/null)"
          STATUS_OK=0; EXIT_CODE=1
      fi
  else
      logecho "[WARN]  postgresql-18 no activo. Skip DB este run."
  fi
else
...
if [ -n "${LARAVEL_DIR:-}" ]; then
  ...
  then
      ...
      logecho "[ERROR] [files] tar falló. Detalle: $(cat "$TMP_DIR/tar.err" 2>/dev/null)"
      STATUS_OK=0; EXIT_CODE=1
  fi
else
...
if [ -z "$FILTERED" ]; then
  logecho "[WARN]  No se localizó ningún config sensible. .keyring vacío skip."
else
  ...
  if [ -s "$KEYRING" ]; then
      ...
      logecho "[ERROR] [keyring] GPG/tar falló. tar=$(cat "$TMP_DIR/tark.err" 2>/dev/null) gpg=$(cat "$TMP_DIR/gpg.err" 2>/dev/null)"
      STATUS_OK=0; EXIT_CODE=1
  fi
fi
...
logecho "============================================================"
if [ "$STATUS_OK" = "1" ]; then
  logecho " [OK] BACKUP COMPLETADO ($TAG) dur=${DUR}s"
else
  logecho " [WARN]  BACKUP FINALIZADO con fallos ($TAG) dur=${DUR}s — revisa $LOG_FILE"
fi
