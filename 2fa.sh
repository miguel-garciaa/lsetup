...
echo "=========================================================================="
if [ "$1" = "--off" ]; then
  echo "   DESACTIVANDO 2FA SSH"
else
  echo "   ACTIVANDO 2FA SSH (GOOGLE AUTHENTICATOR)"
fi
echo "=========================================================================="
...
if [ ! -f "$USER_HOME/.ssh/authorized_keys" ] || [ ! -s "$USER_HOME/.ssh/authorized_keys" ]; then
  echo "[WARN] ALERTA: '$SSH_USER' no tiene claves en authorized_keys."
...
if ! grep -Eq '^#\s*auth\s+substack\s+password-auth' /etc/pam.d/sshd; then
  echo "   [WARN]  AVISO: no se encontró 'auth substack password-auth' en /etc/pam.d/sshd."
