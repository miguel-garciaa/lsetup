...
logecho "============================================================"
logecho " 🔍 VERIFICACIÓN BACKUPS ($TAG) empezando..."
logecho "============================================================"

if [ ! -f "$KEY_FILE" ]; then
    logecho "❌ No existe passphrase $KEY_FILE. No puedo test .keyring."
fi
...
verify_one() {
    ...
    case "$base" in
        *.db)
            ...
            if gzip -t "$f" 2>/dev/null && gunzip -c "$f" 2>/dev/null | pg_restore --list >/dev/null 2>&1; then
                OK=$((OK+1))
                logecho "  ✅ $base (gzip+pg_restore --list OK)"
            else
                FAIL=$((FAIL+1))
                logecho "  ❌ $base (gzip -t o pg_restore --list FALLÓ)"
            fi
            ;;
        *.dat)
            if gzip -t "$f" 2>/dev/null && tar -tzf "$f" >/dev/null 2>&1; then
                OK=$((OK+1))
                logecho "  ✅ $base (gzip+tar header OK)"
            else
                FAIL=$((FAIL+1))
                logecho "  ❌ $base (gzip -t o tar -tzf FALLÓ)"
            fi
            ;;
        *.keyring)
            ...
            if [ -f "$KEY_FILE" ]; then
                ...
                then
                    OK=$((OK+1))
                    logecho "  ✅ $base (GPG decrypt + tar -t OK)"
                else
                    FAIL=$((FAIL+1))
                    logecho "  ❌ $base (GPG decrypt o tar -t FALLÓ — ¿passphrase incorrecta?)"
                fi
            else
                # Sin key: solo test que el container GPG es parseable (no contenido).
                if gpg --list-packets "$f" >/dev/null 2>&1; then
                    OK=$((OK+1))
                    logecho "  ✅ $base (GPG packets OK, contenido no validado sin passphrase)"
                else
                    FAIL=$((FAIL+1))
                    logecho "  ❌ $base (GPG --list-packets FALLÓ: container corrupto)"
                fi
            fi
            ;;
        *.sha256)
    ...
    if [ -f "$sha_file" ] && [[ "$base" != *.sha256 ]]; then
        if (cd "$(dirname "$f")" && sha256sum -c "$(basename "$sha_file")" >/dev/null 2>&1); then
            ...
            logecho "  ❌ $base (SHA256 mismatch respecto $sha_file)"
        fi
    fi
}
...
logecho "============================================================"
if [ "$FAIL" = "0" ]; then
    logecho " ✅ VERIFY OK: $OK/$TOTAL snapshots íntegros"
else
    logecho " ⚠️  VERIFY PARCIAL: OK=$OK FAIL=$FAIL TOTAL=$TOTAL (revisar $LOG_FILE)"
fi
