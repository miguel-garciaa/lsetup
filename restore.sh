...
if [ ! -f "$CONF" ]; then
  echo "[ERROR] Falta $CONF. Ejecuta: sudo bash backup-install.sh" >&2
  exit 1
fi
...
if [ ! -d "$TIER_DIR" ]; then
  log "[ERROR] $TIER_DIR no existe. Aborto."; exit 1
fi
...
if [ -z "$TAG" ] || ! [[ "$TAG" =~ ^[0-9]{8}-[0-9]{4}$ ]]; then
  log "[ERROR] TAG '$TAG' formato inválido. Use YYYYMMDD-HHMM. Aborto."; exit 1
fi
...
for ext in db dat keyring; do
  if [ -f "$TIER_DIR/state-$TAG.$ext" ]; then
      AVAIL[$ext]=1
      echo "  [OK] state-$TAG.$ext EXISTE ($(numfmt --to=iec "$(stat -c %s "$TIER_DIR/state-$TAG.$ext")" 2>/dev/null))"
  else
      AVAIL[$ext]=0
      echo "  [WARN]  state-$TAG.$ext AUSENTE"
  fi
done

if [ "${AVAIL[db]:-0}" = "0" ] && [ "${AVAIL[dat]:-0}" = "0" ] && [ "${AVAIL[keyring]:-0}" = "0" ]; then
  log "[ERROR] TAG $TAG no tiene ningún snapshot en $TIER_DIR. Aborto."; exit 1
fi
...
case "$WHAT" in
  ...
  *) log "[ERROR] Opción '$WHAT' inválida. Aborto."; exit 1;;
esac

# Validar que exista lo pedido.
[ "$RESTORE_DB" = "1" ] && [ "${AVAIL[db]:-0}" = "0" ] && { log "[ERROR] Pediste DB pero .db ausente."; exit 1; }
[ "$RESTORE_FILES" = "1" ] && [ "${AVAIL[dat]:-0}" = "0" ] && { log "[ERROR] Pediste FILES pero .dat ausente."; exit 1; }
[ "$RESTORE_SECRETS" = "1" ] && [ "${AVAIL[keyring]:-0}" = "0" ] && { log "[ERROR] Pediste SECRETS pero .keyring ausente."; exit 1; }

# ------------------------------------------------------------------------------
# PASO 4: Doble confirmación — typear "RESTORE"
# ------------------------------------------------------------------------------
echo ""
echo "[WARN]  ATENCIÓN — destruirá datos actuales del server en las áreas elegidas."
...
if [ "$RESTORE_DB" = "1" ]; then
  ...
  if [ -z "${DB_NAME:-}" ] || [ -z "${DB_USER:-}" ]; then
      log "   [WARN]  Sin creds DB en .env actual. Pregunta manualmente."
  ...
  if ! gzip -t "$SRC" 2>"$BACKUP_ROOT/tmp/rdb.err"; then
      log "   [ERROR] gzip -t falla: $(cat "$BACKUP_ROOT/tmp/rdb.err" 2>/dev/null)"; fail=1
  elif ! gunzip -c "$SRC" | pg_restore --list >/dev/null 2>&1; then
      log "   [ERROR] pg_restore --list falla (dump corrupto)"; fail=1
  else
      ...
      then
          ...
          log "   [WARN]  Backup DB actual falló. Continúo pero asume riesgo."
      fi
      ...
      then
          log "   [OK] DB restaurada."
          ok=1
      else
          log "   [WARN]  pg_restore terminó con avisos (común con --clean). Detalle:"
...
if [ "$RESTORE_FILES" = "1" ]; then
  ...
  if [ -z "$LARAVEL_DIR" ]; then
      log "   [WARN]  No se detectó LARAVEL_DIR actual."
      read -rp "   Ruta destino (/var/www/<dir>): " LARAVEL_DIR
  fi
  if [ ! -d "$LARAVEL_DIR" ]; then
      log "   [ERROR] $LARAVEL_DIR no existe. Abortar files."; fail=1
  elif ! gzip -t "$SRC" 2>"$BACKUP_ROOT/tmp/rf.err"; then
      log "   [ERROR] gzip -t falla: $(cat "$BACKUP_ROOT/tmp/rf.err" 2>/dev/null)"; fail=1
  elif ! tar -tzf "$SRC" >/dev/null 2>&1; then
      log "   [ERROR] tar header corrupto"; fail=1
  else
      ...
      if tar -xzf "$SRC" -C /var/www 2>"$BACKUP_ROOT/tmp/rf2.err"; then
          ...
          log "   [OK] FILES restaurados a $LARAVEL_DIR (owner $LARAVEL_USER:$LARAVEL_USER, SELinux relabeled)."
      else
          log "   [ERROR] tar -xzf falló: $(cat "$BACKUP_ROOT/tmp/rf2.err" 2>/dev/null)"; fail=1
      fi
  fi
fi
...
if [ "$RESTORE_SECRETS" = "1" ]; then
  ...
  if [ ! -f "$KEY_FILE" ]; then
      log "   [ERROR] No existe passphrase $KEY_FILE. Imposible desencriptar .keyring."; fail=1
  elif ! gpg --list-packets "$SRC" >/dev/null 2>&1; then
      log "   [ERROR] GPG packets corruptos: $SRC"; fail=1
  else
      # Extraer a tmp.
      ...
      then
          ...
          sshd -t 2>"$BACKUP_ROOT/tmp/sshd_test.err" || { log "   [WARN]  sshd -t falla tras restore: $(cat "$BACKUP_ROOT/tmp/sshd_test.err" 2>/dev/null)"; }
          visudo -c 2>/dev/null || { log "   [WARN]  visudo -c falla tras restore."; }
          nginx -t 2>/dev/null || { log "   [WARN]  nginx -t falla tras restore."; }
          log "   [OK] SECRETS restaurados. Reinicia servicios cd:"
          log "        sudo systemctl restart sshd nginx redis postgresql-18 fail2ban auditd octane"
          ok=1
      else
          log "   [ERROR] Decrypt/extract falló. gpg=$(cat "$BACKUP_ROOT/tmp/rs1.err" 2>/dev/null) tar=$(cat "$BACKUP_ROOT/tmp/rs2.err" 2>/dev/null)"; fail=1
      fi
...
log "============================================================"
if [ "$fail" = "0" ] && [ "$ok" = "1" ]; then
  log " [OK] RESTORE COMPLETADO ($TAG tier=$TIER what=$WHAT)"
  ...
  log " [WARN]  RESTORE PARCIAL/FALLIDO. fail=$fail ok=$ok"
